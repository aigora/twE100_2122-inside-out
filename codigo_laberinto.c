...
void crear_laberinto (char matriz[F][C], char matrizaux[F][C])
{
    ...
    copiamatriz(i, j, matriz[0], matrizaux[0]);
    ...
    do
    {
        ...
        if (f==120)
        {
            copiamatriz(i, j, matrizaux[0], matriz[0]);
            imprime_matriz(matriz);
        } else
        {
...
void imprime_estadisticas()
{
    ...
    printf("\n");
    printf("Si desea limpiar el historial pulse F\n");
    scanf(" %c", &limp);
...
void limpiar_historial()
{
    int i;
    FILE *f;
    f=fopen("Estadistica.csv", "w");
    for ( i = 0; i < 6; i++)
    {
        fprintf(f, " 10000; 10000\n");
    }
    fclose(f);
}
